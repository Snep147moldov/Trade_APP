...
DEFAULT_SETTINGS = {
    ...
    "breakeven_at_r": 1.3,
    # СКОЛЬКО прибыли запирает этот перенос, в R. 0 = стоп ровно на входе, то
    # есть сделка, дошедшая до +1.3R и развернувшаяся, закрывается в 0.00 EUR:
    # 4 сигнала из 11 в пачке 07.09 (#516, #519, #520, #522) — это 4 x ~16 EUR
    # бумажной прибыли, отданных обратно. Ненулевой lock оставляет часть себе.
    # Фактическое значение ограничено ценой закрытия бара: стоп по ту сторону
    # рынка брокер отклонит.
    ...
    "expiry_bars": 96,
    # выход по времени, но ТОЛЬКО когда сделка в плюсе (0 = выкл). Отдельно от
    # expiry_bars, который закрывает по сроку независимо от знака. Отвечает на
    # наблюдение: сделка стоит в плюсе почти весь день, а к вечеру отдаёт всё
    # обратно. Здесь она забирает то, что есть, и уходит.
    "profit_exit_bars": 0.0,
    "profit_exit_min_r": 0.3,      # минимальный плюс для такого выхода, в R
    # ВНИМАНИЕ: частичная фиксация НЕ отправляется брокеру — close_position
    # закрывает позицию целиком, частичного закрытия в MT5-слое нет. Включённой
    # она рисует в базе прибыль, которую брокер не платил. Пока не реализовано
    # частичное закрытие, на сделках с живой позицией она игнорируется.
    ...
    # за сколько минут до пятничного закрытия ПРИНУДИТЕЛЬНО закрывать открытые
    # позиции. weekend_guard_min только не даёт открывать новые — уже открытые
    # он не трогает, и они висели через выходные до понедельника, где гэп на
    # открытии может перескочить стоп. 0 = не закрывать. Крипта не затрагивается.
    "weekend_close_min": 0.0,
    ...
    "quiet_resume_hour": 1,
    # часы UTC, в которые сигналы не создаются вовсе. Отдельно от ночной паузы:
    # это не «спит рынок», а «в эти часы стратегия систематически теряет».
    # Замер: UTC 7-11 (10-14 по Бухаресту, утро Лондона) убыточны в ОБЕИХ
    # проверках — там рынок как раз формирует тренд, а формула ставит на
    # возврат к среднему. Пусто = фильтр выключен.
    "blocked_hours_utc": [],
    # часы UTC, в которые формула работает В ИСХОДНОМ виде (по тренду), а не
    # с исправленными знаками. Утро Лондона — единственное время, когда рынок
    # действительно формирует тренд, и там инверсия работает против себя.
    # Замер: часы 9 и 10 UTC переходят с -0.399 и -0.347 на +0.173 и +0.225,
    # общее E[R] с +0.092 до +0.149 (PF 1.16 -> 1.26), причём на невиденной
    # половине прирост БОЛЬШЕ, чем на видимой. Пусто = один режим на сутки.
    "trend_hours_utc": [],
