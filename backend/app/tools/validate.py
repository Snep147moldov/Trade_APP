...
async def main() -> None:
    ...
    # Сколько сделке вообще позволено жить. Все прежние прогоны шли с 96
    # барами, то есть практически без ограничения, — этот параметр ни разу не
    # проверялся отдельно.
    compare("СРОК ЖИЗНИ СДЕЛКИ (expiry_bars)",
            [(f"{e} баров", {"expiry_bars": e})
             for e in (24, 36, 48, 72, 96)])

    # Самый короткий край: «как только сделка в плюсе — закрыть». Закрыть и
    # тут же открыть заново по той же цене — это та же позиция минус ещё один
    # спред, поэтому смысл идеи целиком в раннем выходе, а не в перезаходе.
    # 15 минут на часовых барах не выражаются вовсе: минимум — один бар.
    compare("ЗАБРАТЬ ПЛЮС СРАЗУ (самый короткий выход)",
            [("выкл", {"profit_exit_bars": 0}),
             ("1 бар, если > +0.1R",
              {"profit_exit_bars": 1, "profit_exit_min_r": 0.1}),
             ("1 бар, если > +0.3R",
              {"profit_exit_bars": 1, "profit_exit_min_r": 0.3}),
             ("2 бара, если > +0.3R",
              {"profit_exit_bars": 2, "profit_exit_min_r": 0.3})])
