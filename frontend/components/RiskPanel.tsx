...
const SEV_STYLE: Record<string, string> = {
  critical: "border-neg/30 bg-neg/5",
            ...
            <div key={i} className={`rounded-2xl border p-3 ${SEV_STYLE[a.severity]}`}>
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className={`rounded-full text-[9px] ${
                  a.severity === "critical" ? "bg-neg/10 text-neg" :
                  a.severity === "warning" ? "bg-warn/15 text-warn" : ""}`}>
              ...
              <p className="mt-1 text-[11px] font-medium text-brand-ink">→ {a.action}</p>
            </div>
          ))}
        </div>
      )}
      {data.alerts.length === 0 && (
        <p className="rounded-2xl bg-pos/5 p-3 text-xs text-pos">
              ...
              <TableBody>
                {data.positions.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className={`text-xs font-semibold ${p.direction === "BUY" ? "text-pos" : "text-neg"}`}>
                    ...
                    <TableCell className={`text-right text-xs tabular-nums ${
                      (p.r_now ?? 0) >= 0 ? "text-pos" : "text-neg"}`}>
                      {p.r_now != null ? `${p.r_now > 0 ? "+" : ""}${p.r_now.toFixed(2)}R` : "—"}
                    </TableCell>
                    <TableCell className={`text-right text-xs tabular-nums ${
                      (p.floating_eur ?? 0) >= 0 ? "text-pos" : "text-neg"}`}>
